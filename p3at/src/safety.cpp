...
int main(int argc, char **argv)
{
	...
	// laser message
	// in MobileSim not available!)
	ros::Subscriber sub_laser = m.subscribe("/scan", 1, laserCallback);
	...
	while (ros::ok()) {
		if((stopBackward && vel.linear.x < 0.01) || ((stopForward || stopForwardSonar || stopForwardLaser) && vel.linear.x > -0.01))	{
			...
			vel.linear.x = vel.linear.x * SLOWDOWN_FACTOR;
			vel.linear.y = vel.linear.y * SLOWDOWN_FACTOR;
			vel.linear.z = vel.linear.z * SLOWDOWN_FACTOR;
			vel.angular.x = vel.angular.x * SLOWDOWN_FACTOR;
			vel.angular.y = vel.angular.y * SLOWDOWN_FACTOR;
			vel.angular.z = vel.angular.z * SLOWDOWN_FACTOR;
		}
