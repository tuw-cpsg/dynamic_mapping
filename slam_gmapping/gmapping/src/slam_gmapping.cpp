  ...
  laser_count_(0), transform_thread_(NULL)
{
  ...
  if(!private_nh_.getParam("map_frame", map_frame_))
    map_frame_ = "slam_map";  //by sbrugger
  ...
  //by sbrugger  
  if(!private_nh_.getParam("publishLaserMap", publish_laser_map_))
    publish_laser_map_ = 1;
  ...
  sst_laser_ = node_.advertise<nav_msgs::OccupancyGrid>("laser_map", 1, true);	//by sbrugger
  sstm_laser_ = node_.advertise<nav_msgs::MapMetaData>("laser_map_metadata", 1, true);	//by sbrugger
  sst_probabilistic_ = node_.advertise<nav_msgs::OccupancyGrid>("slam_map", 1, true);	//by sbrugger
  sstm_probabilistic_ = node_.advertise<nav_msgs::MapMetaData>("slam_map_metadata", 1, true);	//by sbrugger
...
SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
  ...
  GMapping::SensorMap smap;
	GMapping::SensorMap lasermap; //by sbrugger
  smap.insert(make_pair(gsp_laser_->getName(), gsp_laser_));
	lasermap.insert(make_pair(gsp_laser_->getName(), gsp_laser_)); //by sbrugger
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
  ...
  if(!got_map_) {
    ...
    probabilistic_map_.map.info.resolution = delta_;			//by sbrugger
    probabilistic_map_.map.info.origin.position.x = 0.0;		//by sbrugger
    probabilistic_map_.map.info.origin.position.y = 0.0;		//by sbrugger
    probabilistic_map_.map.info.origin.position.z = 0.0;		//by sbrugger
    probabilistic_map_.map.info.origin.orientation.x = 0.0;	//by sbrugger
    probabilistic_map_.map.info.origin.orientation.y = 0.0;	//by sbrugger
    probabilistic_map_.map.info.origin.orientation.z = 0.0;	//by sbrugger
    probabilistic_map_.map.info.origin.orientation.w = 1.0;	//by sbrugger
  

		laser_map_.map.info.resolution = delta_;		//by sbrugger
		laser_map_.map.info.origin.position.x = 0.0;	//by sbrugger
		laser_map_.map.info.origin.position.y = 0.0;	//by sbrugger
		laser_map_.map.info.origin.position.z = 0.0;	//by sbrugger
		laser_map_.map.info.origin.orientation.x = 0.0; //by sbrugger
		laser_map_.map.info.origin.orientation.y = 0.0; //by sbrugger
		laser_map_.map.info.origin.orientation.z = 0.0; //by sbrugger
		laser_map_.map.info.origin.orientation.w = 1.0; //by sbrugger
	}
  ...
  GMapping::ScanMatcherMap smap(center, xmin_, ymin_, xmax_, ymax_, 
                                delta_);
  GMapping::ScanMatcherMap lasermap(center, xmin_, ymin_, xmax_, ymax_, 
                                delta_);
      ...
      n = n->parent)
  {
    ...
    matcher.computeActiveArea(smap, n->pose, &((*n->reading)[0]));
		...
		if(count == 0)
    	matcher.registerScan(smap, n->pose, &((*n->reading)[0]), true);
		else
...
//if(map_.map.info.width == (unsigned int) 0 || map_.map.info.height == (unsigned int) 0) {
	...
	probabilistic_map_.map.info.width = smap.getMapSizeX();		//by sbrugger
    probabilistic_map_.map.info.height = smap.getMapSizeY();		//by sbrugger
    probabilistic_map_.map.info.origin.position.x = xmin_;		//by sbrugger
    probabilistic_map_.map.info.origin.position.y = ymin_;		//by sbrugger
    probabilistic_map_.map.data.resize(probabilistic_map_.map.info.width * probabilistic_map_.map.info.height);	//by sbrugger

	laser_map_.map.info.width = lasermap.getMapSizeX();		//by sbrugger
    laser_map_.map.info.height = lasermap.getMapSizeY();		//by sbrugger
    laser_map_.map.info.origin.position.x = xmin_;		//by sbrugger
    laser_map_.map.info.origin.position.y = ymin_;		//by sbrugger
    laser_map_.map.data.resize(laser_map_.map.info.width * laser_map_.map.info.height);	//by sbrugger

    ROS_DEBUG("map origin: (%f, %f)", map_.map.info.origin.position.x, map_.map.info.origin.position.y);
  }
// sbrugger do not resize map
    ...
    for(int y=0; y < smap.getMapSizeY(); y++)
    {
      ...
      if(occ < 0)	
        ...
        map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = -1;
 		probabilistic_map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = -1;	//by sbrugger
	  }
      else if(occ > occ_thresh_)
      {
        probabilistic_map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = (int)round(occ*100.0);	//by sbrugger
      ...
      else	
	  {
		...
		if(smap.cell(p).getDynamic() == 0)	{
			laser_map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 0;			//by sbrugger
		}
		else
			laser_map_.map.data[MAP_IDX(map_.map.info.width, x, y)] = 100;       //by sbrugger  
		  
		// reset dynamic value
			smap.cell(p).setDynamic(0);
		}
  }
  got_map_ = true;
	//lasermap.clear();
  ...
  if(publish_laser_map_ != 0)	{
		sst_laser_.publish(laser_map_.map); 		//by sbrugger
		sstm_laser_.publish(laser_map_.map.info);	 //by sbrugger
	}
  sst_probabilistic_.publish(probabilistic_map_.map);		//by sbrugger
  sstm_probabilistic_.publish(probabilistic_map_.map.info);	//by sbrugger
}
